...
@Component({
  selector: 'app-doch-ovdim-pnuim',
  templateUrl: './doch-ovdim-pnuim.component.html',
  styleUrls: ['./doch-ovdim-pnuim.component.scss']

})
export class DochOvdimPnuimComponent extends BaseReportComponent implements OnInit {
  reportName: string = "tvr_r_ovdim_pnium";
    ...
    if (this.dochForm.valid) {
      if (validResult != '') {
        ...
        if (!this.dtDate) {
          ...
          let reportParameters: string = `Date=${moment(this.dtDate).format("YYYY-MM-DD")}&YechidaId=${yechida}`;
